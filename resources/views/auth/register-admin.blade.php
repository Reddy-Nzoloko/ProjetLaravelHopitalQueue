<x-guest-layout>
    <div class="min-h-screen bg-slate-900 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
            <h2 class="text-center text-3xl font-extrabold text-white">Créer un Administrateur</h2>
            <p class="mt-2 text-center text-sm text-slate-400">Ajouter un administrateur pour l'hôpital</p>
            <p class="mt-4 text-center text-xl font-semibold text-emerald-400">{{ $hopital->nom }}</p>
        </div>

        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-slate-800 py-8 px-4 shadow-2xl sm:rounded-2xl sm:px-10 border border-slate-700">
                <form method="POST" action="{{ route('register.admin.store', $hopital) }}" class="space-y-6">
                    @csrf
                    ...
                    <div class="flex items-center justify-between gap-4 mt-6">
                        <a href="{{ route('hopitaux.index') }}" class="text-slate-400 hover:text-slate-300 transition">
                            ← Retour
                        </a>
